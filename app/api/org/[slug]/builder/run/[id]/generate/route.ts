/**
 * app/api/org/[slug]/builder/run/[id]/generate/route.ts
 * V25: POST — Generate BuildArtifacts for a run.
 *
 * Deterministic generation: builds impl plan, prompt pack, checklist
 * from the run's inputJson + mode. No LLM required for plan_only.
 *
 * Transitions run: draft → review
 */

import { NextRequest, NextResponse } from "next/server";
import { checkBuilderAccess, ALL_ARTIFACT_TYPES, isValidTransition } from "@/lib/builder/builder-guard";
...
function buildPromptPack(input: any): string {
    ...
    const pains = Array.isArray(input.pains) ? input.pains : [input.pains ?? "Automação"];
    return [
        `# Prompt Pack — ${company}`,
        "",
        "## Context summary",
...
export async function POST(
    ...
    // Advance run to "review"
    await (prisma as any).buildRun.update({
        where: { id },
        data: { status: "review", outputJson: JSON.stringify({ artifactCount: created.length, types: created.map((a: any) => a.type) }), updatedAt: new Date() },
    });

    logger.info("[Builder] Artifacts generated", { runId: id, count: created.length });

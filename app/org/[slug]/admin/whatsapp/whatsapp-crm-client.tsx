...
import { readApiData } from "./_components/api-envelope";

export function WhatsAppCRMClient({ slug }: { slug: string }) {
    ...
    useEffect(() => {
        fetch(`/api/org/${slug}/whatsapp/stats`)
            .then((res) => readApiData<{ total: number; unassigned: number; breached: number }>(res, "Falha ao carregar estatisticas"))
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <main className="flex-1 overflow-hidden relative">
                    ...
                    <div className="h-full w-full">
                        {activeTab === "inbox" && <InboxTab slug={slug} initialConversationId={requestedConversationId} />}
                        {activeTab === "campaigns" && <CampaignsTab slug={slug} />}
                        {activeTab === "templates" && <TemplatesTab slug={slug} />}

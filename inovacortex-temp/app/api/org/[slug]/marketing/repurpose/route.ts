...
export async function POST(req: NextRequest, { params }: Params) {
    ...
    if (!org || org.id !== plan.orgId) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
...
export async function PATCH(req: NextRequest, { params }: Params) {
    ...
    if (!org || org.id !== artifact.orgId) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
